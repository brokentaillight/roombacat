...
var Pather = {
	...
	*/
	moveTo: function (x, y, retry, clearPath, pop) {
		...
		while(me.hpmax === Config.meHPmaxOrig && me.name === Config.BObarb.ldr) {
			if(me.area != 123 && me.area != 124) break;
		...
		// deeWorkon
		// before I move, is everyone near me (the leader)?
		/*if(me.name === Config.BObarb.ldr && !me.inTown) {
			for(var _i = 0; _i < Config.BObarb.troops.length; _i += 1) {
				if(Config.BObarb.troops[_i] !== Config.BObarb) {
					this.myChkIfPeepIsNear(Config.BObarb.troops[_i]);
				}
			}
		}*/
...
MainLoop:
	...
	*/
	...
	//
	myChkIfPeepIsNear: function (name) {		// dEdits: for making sure team is nearby
		var unit = getUnit(0, name);
		if(unit === undefined && me.inTown) return;
		delay(rand(3000,4000));
		unit = getUnit(0, name);
		if(unit === undefined && !me.inTown) {
			Town.goToTown();
			Town.doChores();
			Town.move("portalspot");
			while(getUnit(0, name) === undefined) { delay(rand(1000,1500)); }
			Pather.usePortal(null, me.name);
			Pather.makePortal();
			unit = getUnit(0, name);
			while(unit.area !== me.area) { 
				delay(rand(1200,2100)); 
				unit = getUnit(0, name);
			}
			return;
		}
		me.overhead("unit: " + unit.name + "| unit.area: " + unit.area + " | distance: " + getDistance(me, unit));
		delay(250);
		if(unit.area !== me.area) {
			Town.goToTown();
			Town.move("portalspot");
			me.overhead("A peep is not in my area, so I will hang here.");
			unit = getUnit(0, name);
			while(unit.area !== me.area) { 
				delay(rand(1200,2100)); 
				unit = getUnit(0, name);
		...
		unit = getUnit(0, name);
		while(getDistance(me, unit) > 45 && (unit.area === me.area)) {
			this.clear(10, spectype);
			delay(rand(300,1100));
			unit = getUnit(0, name);
		}
