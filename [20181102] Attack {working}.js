...
var Attack = {
	...
	clearLevel: function (spectype, _wait) {	// wait: tele slowly and wait for team
		var _meX, _meY;
		
		if (Config.MFLeader) {
			Pather.makePortal();
			me.overhead("clearlevel " + getArea().name);
		}
		...
		while (rooms.length > 0) {
			...
			if (result) {
				...
				if(_wait) {
					while(!myTroops.TroopsNearMe()) {
						Attack.clear(5, 0, 0, false, true);
						delay(250);
						Pather.moveTo(_meX, _meY);
						break;
					}
				}
			}
