...
function Follower() {
	...
	while (Misc.inMyParty(Config.Leader)) {
		...
		if (me.classid == 4 && me.weaponswitch != 0) Precast.weaponSwitch(0); // verifies if bo barb has weapon slot 0 engaged
		if (!me.inTown) {
			...
			if (((leader.area === 132 || leader.area === 109) && me.area === 131) && !me.getSkill(54,1)) Town.goToTown();	// attemot to fix the flwrs gettin' stuck in throne
																															// while the leader is in the chamber
			
			if (leaderUnit && getDistance(me.x, me.y, leaderUnit.x, leaderUnit.y) <= 100 ) { // dEdit: changed <= 60 to != 0
...
MainLoop:
	...
	while (myTroops.LdrInMyArea()) { delay(rand(1200,2000));	}
	Town.goToTown(5);
	return true;
}
function d2_Pindleskin() {
	...
	while(!(myTroops.TroopsInMyArea() >= (Config.Troops.length - 1))) { 		// check if everyone is in the same area & near, if not pause 'til they are
		myQuietVoice("...We'll stay put, 'til they return.");
